...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("SimilarityMatrixTemporalDownsampling");

  // ---------------------------------------------------------------------------
  // Initialize ttkSimilarityMatrixTemporalDownsampling module (adjust
  // parameters)
  // ---------------------------------------------------------------------------
  auto similarityMatrixTemporalDownsampling
    = vtkSmartPointer<ttkSimilarityMatrixTemporalDownsampling>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // similarityMatrixTemporalDownsampling->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkSimilarityMatrixTemporalDownsampling filter
      similarityMatrixTemporalDownsampling->SetInputDataObject(
        i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    similarityMatrixTemporalDownsampling->SetInputArrayToProcess(
      i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkSimilarityMatrixTemporalDownsampling filter
  // ---------------------------------------------------------------------------
  similarityMatrixTemporalDownsampling->Update();
  ...
  if(!outputPathPrefix.empty()) {
    for(int i = 0;
        i < similarityMatrixTemporalDownsampling->GetNumberOfOutputPorts();
        i++) {
      auto output
        = similarityMatrixTemporalDownsampling->GetOutputDataObject(i);
      auto writer
