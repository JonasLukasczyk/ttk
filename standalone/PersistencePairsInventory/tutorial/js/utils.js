...
function getTimeByIndex(i) {
	...
	if ( Window.PPI['image-object']['fieldData'].hasOwnProperty("Time") ) {
		v = Window.PPI['image-object']['fieldData']["Time"].data[i] ;
    }
    return v ;
}
...
function getTimeArray() {
    var v = undefined ;
	if ( Window.PPI['image-object']['fieldData'].hasOwnProperty("Time") ) {
        v = Window.PPI['image-object']['fieldData']["Time"].data;
        ans = []
        for (var i = 0; i < v.length; i ++) {
            ans.push(v[i]) ;
        }
...
function getThresholdByIndex(i) {
	...
	if ( Window.PPI['image-object']['fieldData'].hasOwnProperty("PersistenceThresholds") ) {
		v = Window.PPI['image-object']['fieldData']["PersistenceThresholds"].data[i] ;
    }
    return v ;
}
...
function getThresholdArray() {
    var v = undefined ;
	if ( Window.PPI['image-object']['fieldData'].hasOwnProperty("PersistenceThresholds") ) {
        v = Window.PPI['image-object']['fieldData']["PersistenceThresholds"].data;
        ans = []
        for (var i = 0; i < v.length; i ++) {
            ans.push(v[i]) ;
        }
...
function getScalarArray(id=-1) {
    ...
    if ( Window.PPI['image-object']['fieldData'].hasOwnProperty("ScalarBounds")) {
        ...
        var gap = (fieldData['ScalarBounds'].data[1] - fieldData['ScalarBounds'].data[0]) / (Window.PPI['histogram-height'] - 1)
        if ( id > -1) {
            return gap * id + fieldData['ScalarBounds'].data[0] ;
        } else {
            var ans = [] ;
            for (var i = 0; i < Window.PPI['histogram-height']; i ++) {
                var tmp = gap * i + fieldData['ScalarBounds'].data[0] ;
