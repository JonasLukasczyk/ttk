...
function drawBoxPlot() {
	// redraw the curve
    drawBoxplotCurve(Window.PPI['image-object'].fieldData.PersistenceCurves.nComponents,
    				 Window.PPI['image-object'].fieldData.PersistenceCurves.data);
...
function objectCallback(obj) {
    if (obj.className!=='vtkImageData') {
        alert("MUST be ImageData") ;
    }

    // initialize the global variable
    console.log(obj);
    Window.PPI['image-object'] = obj;
    Window.PPI['histogram-width'] = obj.dimension[0];
    Window.PPI['histogram-height'] = obj.dimension[1];
    // MUST contain one element
    Window.PPI['APPIAttrName'] = Object.keys(obj.pointData)[0];
    Window.PPI['numberOfThreshold-histogram'] = obj.pointData[Window.PPI['APPIAttrName']].nComponents ;
    Window.PPI['numberOfThreshold-boxplot'] = obj.fieldData.PersistenceCurves.nComponents ;
...
function Connect() {
	...
	if (ttk && ttk.socket.readyState !== 3) {
	...
	ttk = new TTK.ttkWebSocketIO();
	ttk.connect(null,PORT);
	ttk.on( 'open',  ()=>ttk.sendString('RequestInputVtkDataSet'));
	ttk.on( 'messageSequence',  msgs=>{
    TTK.vtkDataSet.createFromMessageSequence(msgs).then( objects=>{
        objectCallback(objects[0]);
      });
  });
...
// 		ip = $("#msg-host").val(), false);
	...
	ttk_render = new TTK.ttkWebSocketIO();
	ttk_render.on( 'messageSequence',  msgs=>{
    TTK.vtkDataSet.createFromMessageSequence(msgs).then( objects=>{
        RENDERER.setScene(objects[0]);
      });
  });
	ttk_render.connect(null,PORT_RENDER);
	ttk_render.on( 'open',  ()=>ttk_render.sendString('RequestInputVtkDataSet'));
...
function triggerCtrlV() {
	...
	if (Window.PPI['histogram-mode'] == "multi") {
		...
		drawSDMHistogram(Window.PPI['DEV']? null: (ttk ? ttk.socket: null)) ;
		...
		drawHistogram(parseInt($("#hist-threshold").val()), Window.PPI['DEV']? null: (ttk ? ttk.socket: null)) ;
			...
			if (Window.PPI['histogram-mode'] == "multi") {
				...
				if (selected_sdm) {
					if (i_sdm + 1 <= Window.PPI['sdm-histogram-width'] - 1) {
						...
						jClick("#sdm-hist-bin-" + (g_idx_sdm + 1)) ;
					}
				}
			}
			break;
			...
			if (Window.PPI['histogram-mode'] == "multi") {
				...
				if (selected_sdm) {
					if (j_sdm + 1 <= Window.PPI['histogram-height'] - 1) {
						...
						jClick("#sdm-hist-bin-" + (g_idx_sdm + Window.PPI['sdm-histogram-width'])) ;
					}
				}
			}
...
$("#hist-threshold").change(function () {
    ...
    $("#hist-threshold option:selected").each(function () {
    	if (Window.PPI['histogram-mode'] == "multi") {
	        drawHistogram(parseInt($(this).val()), Window.PPI['DEV']? null: (ttk ? ttk.socket: null));
    	} else {
	        drawSDMHistogram(Window.PPI['DEV']? null: (ttk ? ttk.socket: null));
    	}
	});
...
$("#hist-rescale").unbind().click(function() {
    ...
    if (Window.PPI['histogram-mode'] == "multi") {
	    drawHistogram(parseInt($("#hist-threshold").val()), Window.PPI['DEV']? null: (ttk ? ttk.socket: null), tag=1);
    } else {
	    drawSDMHistogram(Window.PPI['DEV']? null: (ttk ? ttk.socket: null));
    }
}) ;
