...
int ttkCorrespondenceByJacobiSet::ComputeCorrespondences(
  ...
  // auto stackedScalarArray =
  // vtkSmartPointer<vtkDataArray>::Take(scalar0->NewInstance());
  ...
  auto components = vtkSmartPointer<vtkPolyData>::New();
  {
    ...
    this->printMsg(
      msg, 0.1, t.getElapsedTime(), 1, ttk::debug::LineMode::REPLACE);
    ...
    threshold->Update();
    this->printMsg(
      msg, 0.3, t.getElapsedTime(), 1, ttk::debug::LineMode::REPLACE);
    ...
    this->printMsg(
      msg, 0.5, t.getElapsedTime(), 1, ttk::debug::LineMode::REPLACE);
    ...
    this->printMsg(
      msg, 0.7, t.getElapsedTime(), 1, ttk::debug::LineMode::REPLACE);
  ...
  // iterate over features and find for each critical points its corresponding
  // connected component
  {
    ttk::Timer t;
    ...
    // vtkStaticCleanPolyData turns this array into a float array, no matter the
    // original data type
    ...
    this->printMsg(msg, 0.1, t.getElapsedTime(), this->threadNumber_,
    ...
    ttkTypeMacroI(
      ...
      (status = computeComponentIdMap<T0, 0>(
         componentIdToVertexIdMap, ttkUtils::GetPointer<T0>(vidPoints0),
         nPoints0, ttkUtils::GetPointer<float>(vidComponents),
         nComponentVertices, componentIdsData, 0)));
    if(!status)
      return 0;

    this->printMsg(msg, 0.4, t.getElapsedTime(), this->threadNumber_,
                   ttk::debug::LineMode::REPLACE);

    ttkTypeMacroI(
      vidPoints1->GetDataType(),
      (status = computeComponentIdMap<T0, 1>(
         componentIdToVertexIdMap, ttkUtils::GetPointer<T0>(vidPoints1),
         nPoints1, ttkUtils::GetPointer<float>(vidComponents),
         nComponentVertices, componentIdsData, nTuplesPerImage)));
    if(!status)
      return 0;
